// js/gemini.js — Gemini AI integration (key integrated)

const GEMINI_KEY = 'AIzaSyDI0wXGn3NIjGAMIXiswr07YjKy4tyvQuw';
...
async function generateDebrief(sessionData) {
  const { profile, biasScores, decisions, disciplineScore, crashesSurvived, simulationsDone } = sessionData;
...
${STATE.lang === 'hindi' ? 'Respond entirely in Hindi.' : ''}
${STATE.lang === 'punjabi' ? 'Respond entirely in Punjabi.' : ''}`;
...
async function coachChat(userMessage, context) {
  const langInstr = STATE.lang === 'hindi' ? 'Respond entirely in Hindi.' :
                    STATE.lang === 'punjabi' ? 'Respond entirely in Punjabi.' : 'Respond in English.';
...
- ${langInstr}
- User's Fear Profile: ${context.profile || 'Unknown'}
- User's Discipline Score: ${context.disciplineScore}/100`;
